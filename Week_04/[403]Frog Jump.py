...
class Solution(object):
    def canCross(self, stones):
        ...
        hashmap[0].add(0)
        for stone in stones:
            for jumpSize in hashmap[stone]:
                for newJumpSize in range(jumpSize - 1, jumpSize + 2):
                    if newJumpSize > 0 and stone + newJumpSize in hashmap:
                        hashmap[stone + newJumpSize].add(newJumpSize)
        return len(hashmap[stones[-1]]) > 0
